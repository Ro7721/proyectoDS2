...
public class AuthController {
    ...
    public ResponseEntity<?> login(@Valid @RequestBody LoginRequest request) {
        try {
            ...
            error.put("message", "La contraseña ingresada es incorrecta");
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(error);
        } catch (UsernameNotFoundException e) {
            Map<String, String> error = new HashMap<>();
            error.put("error", "EMAIL_NOT_FOUND");
            error.put("message", "El correo no existe o no está registrado");
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
        } catch (Exception e) {
            Map<String, String> error = new HashMap<>();
            error.put("error", "AUTH_ERROR");
            error.put("message", "Ocurrió un error inesperado al iniciar sesión");
    ...
    public ResponseEntity<?> getCurrentUser() {
        ...
        if (auth == null || !auth.isAuthenticated()) {
            ...
            error.put("message", "No tienes autorización para acceder a este recurso");
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(error);
        }
